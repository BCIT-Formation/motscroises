...
import { generateCrossword, getBoundingBox, isGridConnected, rebuildCrossword } from './crossword'
...
// ─── Reconstruction (partage par URL) ──────────────────────────────────────────

describe('rebuildCrossword', () => {
  it('reconstruit à l\'identique une grille générée', () => {
    const original = generateCrossword(WORDS, SIZE)
    const placements = original.placedWords.map(({ word, clue, row, col, dir }) => ({
      word, clue, row, col, dir,
    }))

    const rebuilt = rebuildCrossword(placements, SIZE)
    expect(rebuilt).not.toBeNull()
    expect(rebuilt.grid).toEqual(original.grid)
    expect(rebuilt.numbers).toEqual(original.numbers)
    expect(rebuilt.acrossClues).toEqual(original.acrossClues)
    expect(rebuilt.downClues).toEqual(original.downClues)
  })

  it('refuse un placement hors de la grille', () => {
    expect(
      rebuildCrossword([{ word: 'CHAT', clue: 'x', row: 0, col: 8, dir: 'across' }], 10)
    ).toBeNull()
    expect(
      rebuildCrossword([{ word: 'CHAT', clue: 'x', row: -1, col: 0, dir: 'down' }], 10)
    ).toBeNull()
  })

  it('refuse des lettres en conflit sur une même case', () => {
    expect(
      rebuildCrossword(
        [
          { word: 'CHAT', clue: 'x', row: 5, col: 2, dir: 'across' },
          // T attendu en (5,2) mais la case contient déjà C
          { word: 'TOIT', clue: 'y', row: 5, col: 2, dir: 'down' },
        ],
        10
      )
    ).toBeNull()
  ...
  it('refuse les entrées malformées', () => {
    expect(rebuildCrossword(null, 10)).toBeNull()
    expect(rebuildCrossword([{ word: 'CHAT', row: 0, col: 0, dir: 'diagonal' }], 10)).toBeNull()
    expect(rebuildCrossword([{ word: '', row: 0, col: 0, dir: 'across' }], 10)).toBeNull()
    expect(rebuildCrossword([], 0)).toBeNull()
  })

  it('accepte des mots qui se croisent correctement', () => {
    const rebuilt = rebuildCrossword(
      [
        { word: 'CHAT', clue: 'félin', row: 2, col: 1, dir: 'across' },
        { word: 'HAUT', clue: 'contraire de bas', row: 2, col: 2, dir: 'down' },
      ],
      8
    )
    expect(rebuilt).not.toBeNull()
    expect(rebuilt.placedWords).toHaveLength(2)
    expect(rebuilt.grid[2][2].letter).toBe('H')
    expect(isGridConnected(rebuilt.grid)).toBe(true)
  })
})
